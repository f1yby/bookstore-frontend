...
const Cards = (props: { bookState: BookData[] }) => {
  ...
  return <Row justify={'center'}>{cols}</Row>;
};
...
const CarouselWrapper = (props: { bookState: BookData[] }) => {
  ...
  props.bookState
    ?.slice(1, 5)
    .forEach((item) => items.push(<CarouselItemWrapper book={item} />));
  // if (props.bookState) {
...
const CarouselItemWrapper = (props: { book: BookData }) => {
  return (
    <Row style={{ width: '60vw' }} justify={'center'}>
      <Col style={{ width: '20vw' }}>
        <img
          src={props.book.image}
          alt={'book'}
          width={'100%'}
          onClick={() => PageSwitcher.jumpToDetailByBook(props.book)}
      ...
      <Col style={{ width: '20vw' }}>
        <Row justify={'center'}>
          <Col>
            <Typography.Title>{props.book.name}</Typography.Title>
          </Col>
        </Row>
